...
def analyser_arguments(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="generateur", description="Générateur d'événements de navigation"
    )
    p.add_argument(
        "--debit",
        type=float,
        default=20.0,
        help="événements par seconde RÉELLE (défaut 20)",
    )
    p.add_argument(
        "--duree",
        type=float,
        default=300.0,
        help="durée réelle en secondes (défaut 300)",
    )
    p.add_argument(
        "--graine",
        type=int,
        default=None,
        help="graine aléatoire ; tirée au sort et affichée si absente",
    )
    p.add_argument(
        "--taux-defauts",
        type=float,
        default=0.05,
        help="part des événements défectueux (défaut 0.05)",
    )
    p.add_argument(
        "--proportions",
        type=_proportions,
    ...
    p.add_argument(
        ...
        type=int,
        default=30_000,
        help="rythme SIMULÉ (dossier : 10 000 à 50 000)",
    )
    p.add_argument(
        "--debut",
        type=_date,
        default=Parametres().debut,
        help="début de la simulation, ISO 8601 (passé)",
    )
    p.add_argument(
        "--catalogue",
        default=os.environ.get(
            "CATALOGUE_RAKUTEN", "data/sources/rakuten/rakuten_catalogue_produits.csv"
        ),
    )
    p.add_argument(
        "--clients",
        default=os.environ.get("CLIENTS_OLIST", "data/sources/olist/olist_customers_dataset.csv"),
    )
    p.add_argument(
        "--sans-pause",
        action="store_true",
        help="publie aussi vite que possible, sans respecter le débit",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if fin_simulee > datetime.now(UTC) - timedelta(minutes=5):
        print(
            "Erreur : la simulation dépasserait l'heure réelle ; choisissez un --debut plus ancien.",
            file=sys.stderr,
        )
        return 2
    ...
    print(
        f"graine={graine} événements={total} du {parametres.debut:%Y-%m-%d} au {fin_simulee:%Y-%m-%d %H:%M}"
    )

    from common.bus import (
        Publieur,  # import tardif : les tests n'ont pas besoin de Kafka
    )
